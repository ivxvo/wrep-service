        ...
        reportSaving
            .then((saveResult) => {
                ...
                if(rep.periodType === globalThis.PeriodType.decade) {
                    reportResult.decade.files.push({ fileName: outputFileName, bytes: saveResult.body.buffer, text: rep.textInEmail, ext: rep.ext });
                }
                else if(rep.periodType === globalThis.PeriodType.month) {
                    reportResult.month.files.push({ fileName: outputFileName, bytes: saveResult.body.buffer, text: rep.textInEmail, ext: rep.ext });
                }
            })
            .catch((err) => {
