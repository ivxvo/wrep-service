...
exports.get = async (input) => {
    ...
    let reportSaving, outputFileName, reportResult = {};
    for(let rep of input.reps) {
        let repPeriod = getPeriod(rep.periodType);
        ...
        outputFileName = repPeriod.name + rep.outputFileName + "_" + moment().format("(DDMMYYYY_HHmm)") + ".xlsx";
        ...
        reportSaving
            .then((saveResult) => {
                ...
                if(rep.periodType === globalThis.PeriodType.decade) {
                    reportResult.decade.files.push({ fileName: outputFileName, bytes: saveResult.body });
                }
                else if(rep.periodType === globalThis.PeriodType.month) {
                    reportResult.month.files.push({ fileName: outputFileName, bytes: saveResult.body });
                }
            })
            .catch((err) => {
