...
exports.get = async (input) => {
    ...
    for(let rep of input.reps) {
        ...
        if(!template) {
            console.error(`Шаблон отчёта '${rep.templateName}' не найден.\r\ntemplate:${template}`);
            continue;
        }
        ...
        for(let groupName of rep.unitGroups) {    
            ...
            if(!group) {
                console.error(`Группа объектов '${groupName}' не найдена.`);
                return;
            }
        ...
        try {        
            let report = await runReport(reportRunParams);
            let id = -0;
        }
        catch(e) {
            return;
        ...
        let repdata = await execRequest("report/select_result_rows", {
            tableIndex: 0,
            config: {
                type: "range",
                data: {
        ...
        await execRequest("report/export_result", {
            format: 1,
            compress: 0,
            pageWidth: 0,
            headings: 1,
            attachMap: 0,
            hideMapBasis: 0,
            outputFileName: rep.outputFileName
        });

        console.log("Report saved");

        // reportSaving = makeQueryablePromise(execRequest("report/export_result", { format: 8, outputFileName: rep.outputFileName }));
        // reportSaving
        //     .then(() => {
        //         console.log(`Report saved: ${rep.outputFileName}`);                
        //     })
        //     .catch((err) => {
...
function login() {
    ...
    // запрос токена
    // http://hosting.wialon.com/login.html?client_id=wialon&access_type=-1
